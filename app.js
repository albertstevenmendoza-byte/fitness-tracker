...
- 
- Architecture:
- IronMacro.state   → in-memory state (goals + daily log)
- IronMacro.storage → localStorage read/write helpers
- IronMacro.ui      → DOM rendering helpers
- IronMacro.init()  → boot sequence
- ═══════════════════════════════════════════════════════════
  */
...
console.warn(‘IronMacro: could not save goals’, e);
}
},
...
loadGoals() {
  try {
    ...
    console.warn('IronMacro: could not load goals', e);
  }
},
...
saveDailyLog() {
  try {
    ...
    console.warn('IronMacro: could not save daily log', e);
  }
},
...
loadDailyLog() {
  try {
    ...
    console.warn('IronMacro: could not load daily log', e);
    state.dailyLog = [];
  }
},
...
clearDailyLog() {
  try {
    ...
    console.warn('IronMacro: could not clear daily log', e);
  }
},
...
savePrefs() {
  try {
    ...
    console.warn('IronMacro: could not save prefs', e);
  }
},
...
loadPrefs() {
  try {
    ...
    console.warn('IronMacro: could not load prefs', e);
  }
},
```
...
setTimeout(() => {
  ...
  if (newKey !== state.todayKey) {
    ...
    setTextSafe('headerDate', formatHeaderDate(state.todayKey));
    renderDashboard();
    console.info('IronMacro: New day detected, log reset.');
  }
