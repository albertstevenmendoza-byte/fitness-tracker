...
const html = `<div class="hist"> <header class="hist__head"> <p class="card__eyebrow">History</p> <h2 class="hist__title">${ex.name}</h2> ${pr ?`
<div class="hist__pr">
...
${s.sets.map((set, i) => `<div class="hist__set"> <span class="hist__set-i">S${i + 1}</span> <span class="hist__set-w tnum">${set.weight}</span> <span class="hist__set-x">×</span> <span class="hist__set-r tnum">${set.reps}</span> <span class="hist__set-rpe">RPE ${set.rpe}</span> </div>`).join(’’)}
</div>
...
return `<div class="lift lift--selector"> <div class="lift__intro"> <p class="card__eyebrow">Today's session</p> <h2 class="lift__heading">${tpl.label}</h2> <p class="lift__sub">Pick a day to start. Your sets autosave.</p> </div> <div class="day-grid"> ${tpl.days.map((d, i) =>`
...
return `<div class="spin" data-spin-field="${field}" data-spin-step="${step}" data-spin-decimals="${decimals}" data-spin-value="${value}"> <button class="spin__btn" data-spin-act="dec" aria-label="Decrease ${field}"> <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M5 12h14"/></svg> </button> <div class="spin__value"> <span class="spin__num tnum" data-spin-display>${(+value).toFixed(decimals)}</span> <span class="spin__unit">${unit}</span> </div> <button class="spin__btn" data-spin-act="inc" aria-label="Increase ${field}"> <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg> </button> </div>`;
}
